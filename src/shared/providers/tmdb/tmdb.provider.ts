...
import { TMDBMapper } from "./tmdb.mapper.js";
...
export class TMDBProvider {
    ...
    async getTrending(type: MediaType): Promise<AppMedia[]> {
        try {
            ...
            return data.results.map((m: TMDBRawResult) => TMDBMapper(m));
        } catch (error: any) {
    ...
    async search(name: string): Promise<AppMedia> {
        try {
            ...
            return data.results.map((media: TMDBRawResult) => TMDBMapper(media));
        } catch (error: any) {
            throw new ApiError(
                error.response?.status || 500,
